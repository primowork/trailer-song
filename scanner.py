...
def parse_ifpi_table(html_content: str, target_artist: str) -> tuple[bool, str]:
    ...
    publishers = ["יוניברסל", "וורנר", "סוני", "nmc", "הליקון", "bmg", "universal", "sony", "warner", "מיוצג"]
    
    for row in rows:
        cols = [td.get_text(" ", strip=True) for td in row.find_all(["td", "th"])]
        row_text = " ".join(cols).lower()
        
        # זיהוי סלחני - האם השם מ-iTunes נמצא איפשהו בתוך השורה
        if clean_target in row_text or fuzz.partial_ratio(clean_target, row_text) > 80:
            ...
            # אם לא נמצאה מילה ספציפית אך יש תוכן בעמודת המייצג (אינדקס 4)
            if len(cols) >= 5 and len(cols[4]) > 2 and cols[4] != "-":
                return True, cols[4]
...
def verify_single_track(track: dict) -> tuple[bool, str]:
    ...
    # חיתוך חכם: שולחים לחיפוש רק את 2 המילים הראשונות של האמן כדי לעקוף טעויות כתיב בפדרציה
    ...
    with sync_playwright() as p:
        ...
        try:
            # אסטרטגיה 1: חיפוש אמן חלקי + שיר
            page.goto("https://www.federation.co.il/Index.asp?CategoryID=94", wait_until="domcontentloaded", timeout=15000)
            page.fill('input[name="artName"]', short_artist)
            page.fill('input[name="trkName"]', clean_song)
            
            # פקודה שנועלת את הבוט עד שהרענון (Navigation) של טבלת ה-ASP מסתיים לחלוטין
            with page.expect_navigation(wait_until="domcontentloaded", timeout=15000):
                page.locator('input[type="submit"], input[value="חפש"], button[type="submit"]').first.click()
            
            page.wait_for_timeout(1500) # מרווח ביטחון לטעינת טקסט מלאה
            is_approved, publisher_found = parse_ifpi_table(page.content(), raw_artist)

            # אסטרטגיה 2: אם נכשל, מחפשים את שם השיר בלבד
            if not is_approved:
                page.goto("https://www.federation.co.il/Index.asp?CategoryID=94", wait_until="domcontentloaded", timeout=15000)
