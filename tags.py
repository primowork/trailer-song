...
# ארבע הראשונות נשענות בעיקר על `audio.WEIGHTS`, שטווחיו כוילו מול מדידות
# אמיתיות. שלוש האחרונות נשענות על `audio.TIMBRE_RANGES`, שהקובץ שלו מצהיר
# במפורש שהוא "הערכה ומיועד לכיול על ייצוא אמיתי" — כלומר הן עומדות על שכבת
# הערכה נוספת. ההפרדה כאן ולא בהערה בלבד, כדי ש-`tools/tag_report.py` יוכל
# לדווח עליהן בנפרד ולא לערבב ודאות עם ניחוש.
#
# **חריגה אחת:** ACTION ו-RELENTLESS (ראו `_rules`) דולקות גם על `flux`
# כשמדד ה-onset המכויל לבדו מפספס טראק דחוס-מאסטרינג. זו עדות מוערכת
# בתוך כלל מכויל — פשרה מודעת, לא טעות סיווג.
...
def _rules(values: dict) -> "list[tuple[str, bool]]":
    ...
    # `onset_rate` לבדו מפספס בדיוק את הז'אנר שהוא אמור לתפוס: מאסטרינג
    # דחוס-מדי (המכתש של דראם אנד בייס/EDM כבד) משטח את ה-RMS כמעט לקו
    # ישר, ולכן קפיצות אנרגיה בין פריימים — ההגדרה של onset_rate — כמעט
    # ולא נספרות, למרות שהטראק צפוף ומתקפי ככל שיהיה. **נמדד**: קאבר
    # דראם אנד בייס אגרסיבי במיוחד קיבל ONSET נמוך מספיק כדי ליפול
    # ל-Calm. `flux` (שטף ספקטרלי, ראו את ההערה עליו ב-`audio.py`) נמדד
    # בדיוק בשביל המקרה הזה: הוא לא מסתכל על *עוצמה* משתנה אלא על *תוכן*
    # ספקטרלי משתנה — היי-האטים, הבסים המעוותים וה-drop-ים של הטראק הזה
    # עדיין קופצים שם גם כשה-RMS כבר שטוח. or ולא and: זה מקור שני לאותה
    # עדות ("האם זה עמוס"), לא תנאי נוסף שצריך להתקיים לצידו.
    #
    # הסף (0.55) לא כויל מול ייצוא אמיתי — `flux` נמצא ב-`ESTIMATED_TAGS`
    # בדיוק בגלל זה — ולכן ACTION/RELENTLESS יכולים עכשיו לדלוק על עדות
    # לא-מכוילת. זו פשרה מודעת: העדות הקודמת (onset_rate לבדו) לא סתם
    # לא-מכוילת אלא **ידועה כמפספסת** את המקרה הזה, ועדות משוערת שתופסת
    # אותו עדיפה על עדות מדויקת שמפספסת אותו.
    busy = hits >= 0.55 or (flux is not None and flux >= 0.55)
    return [
        (ACTION, busy and loud >= 0.45),
        # קצב גבוה *בלי* קשת: מכה אחידה מתחילתה ועד סופה. זה ההפך מ-SLOW BURN
        # ולא וריאציה של ACTION, ולכן תגית משלו.
        (RELENTLESS, busy and span <= 0.30),
